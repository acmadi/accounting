...
$excel->getProperties()
  ...
  ->setDescription('Data Pelanggan '.$corp->nama);
...
// semua tabel
$sheet->getStyle('A6:E'.(count($customer) + 5))
  ->applyFromArray($style);
$sheet->getStyle('A6:E'.(count($customer) + 5))
  ->getAlignment()
  ->setWrapText(true);
...
// kolom nama
$sheet->getStyle('C6:C'.(count($customer) + 5))
  ->getAlignment()
  ->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_JUSTIFY);

// kolom alamat
$sheet->getStyle('E6:E'.(count($customer) + 5))
  ->getAlignment()
  ->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_JUSTIFY);
...
foreach ($customer as $data) {
  ...
  $sheet->setCellValue('A'.($i + 5), $i)
    ...
    ->setCellValue('E'.($i + 5), ($data->alamat) ?: '-');
}
unset($style);
